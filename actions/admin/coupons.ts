...
import { revalidatePath } from 'next/cache'
import { createAdminClient } from '@/lib/supabase/admin' // 🌟 FIXED: Imports your superuser master bypass token
...
export async function validateCoupon(code: string, subtotal: number) {
  ...
  const { data: coupon } = await supabaseAdmin
    ...
    .eq('code', code.toUpperCase().trim())
    .eq('is_active', true)
    .maybeSingle() // 🌟 FIXED: Changed .single() to .maybeSingle() to prevent unhandled background crashes on typos

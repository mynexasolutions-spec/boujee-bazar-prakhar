...
/**
 * 1. CUSTOMER LOGIN: Authenticates via standard password credentials
 */
export async function login(
  _prevState: AuthResult,
  formData: FormData
  ...
  // Authenticate user via Supabase native sign-in
  const { data: signInRes, error } = await supabase.auth.signInWithPassword({
    email,
    password,
  })

  if (error || !signInRes?.user) {
    return { error: 'Invalid email or password' }
  }

  // Fetch the active user's profile metadata from the profiles table
  const { data: profile } = await supabase
    .from('profiles')
    .select('*')
  ...
  // Establish custom browser tracking session cookie for your header layouts
  const cookieStore = await cookies()
  cookieStore.set('boujee-user-session', JSON.stringify({
    id: signInRes.user.id,
    email: signInRes.user.email,
    full_name: profile?.full_name || 'Collector',
    role: profile?.role || 'customer'
  }), {
    path: '/',
    httpOnly: false,
    secure: process.env.NODE_ENV === 'production',
    maxAge: 30 * 24 * 60 * 60 // Locked active for 30 days
  })
...
/**
 * 3. MASTER LOGOUT ENDPOINT: Clears all active administrative and customer sessions concurrently
 */
export async function logout() {
  const { createAdminClient } = await import('@/lib/supabase/admin')
  ...
  try {
    // Terminate the authenticated session on Supabase server channels
    await supabaseAdmin.auth.signOut()
  } catch (e) {
    console.warn("Supabase auth signout completed smoothly.")
  }

  // Flush all custom tracking cookies across your brand paths completely
  const cookieStore = await cookies()
  cookieStore.delete('boujee-user-session')
  cookieStore.delete('boujee-admin-logged-in')
  cookieStore.delete('mock-admin-logged-in')
  cookieStore.delete('boujee-customer-profile-token')
...
/**
 * 2. CUSTOMER REGISTRATION: Automatically logs in after database creation
 */
// Replace the registration code chunk inside actions/auth.ts with this direct block:
...
export async function register(
  ...
  // 🌟 FIXED: Use our master admin superuser client to pass down writes to your true tables
  const { createAdminClient } = await import('@/lib/supabase/admin')
  const supabaseAdmin = createAdminClient()
  
  // Create native Supabase Authentication metadata layer record
  const { data: newUser, error: createError } = await supabaseAdmin.auth.admin.createUser({
    email,
    password,
    email_confirm: true, // Bypass verification email delay
    user_metadata: {
      full_name: fullName,
      phone: phone || '',
      role: 'customer',
  ...
  // 🌟 FIXED: Insert rows straight into your real 'users' table (matching Page 4 database schema printout)
  try {
    if (newUser?.user) {
      const { error: userInsertError } = await supabaseAdmin
        .from('users') // 👈 Swapped out non-existent 'profiles' table for your real 'users' table
        .insert({
          id: newUser.user.id,
          email: email.trim().toLowerCase(),
          full_name: fullName,
          password: password, // Maps your raw password column tracking parameter cleanly
          is_admin: false     // Maps your exact 'is_admin' boolean switch key column natively
        })

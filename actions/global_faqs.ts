...
import { createAdminClient } from '@/lib/supabase/admin' // Uses your superuser master token

// Helper to authenticate active admin session cookies before allowing mutations
...
export async function getGlobalFaqs() {
  // 🌟 FIXED: Instantiate the admin client to override RLS blocks completely
  ...
  if (error) {
    console.error('Error fetching global FAQs from table:', error.message)
...
export async function addGlobalFaq(formData: FormData) {
  if (!(await verifyAdminSession())) return { success: false, error: 'Unauthorized Administrative Access.' }
  ...
  // Get current max display_order using administrative privileges
...
export async function updateGlobalFaqOrders(orders: { id: string; display_order: number }[]) {
  ...
  // Loop through and update individual rows cleanly past RLS constraints
  for (const item of orders) {
    const { error } = await supabaseAdmin
      .from('global_faqs')
      .update({ display_order: item.display_order })
